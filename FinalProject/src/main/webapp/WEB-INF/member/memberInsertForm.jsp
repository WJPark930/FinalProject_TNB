...
<form:form modelAttribute="member" method="post" action="insertuser.mb" enctype="multipart/form-data" onsubmit="return validateForm() && validatePassword()">
    ...
    <table border="1">
        <tr>
            <th>* 이메일</th>
            <td>
        <input type="text" id="user_email" name="user_email" value="${user_email}"  onkeyup="checkEmail()" disabled >
        ...
        <!-- Nickname Field in Form -->
           ...
           <tr>
            <th>* 이름</th>
            <td>
                <input type="text" id="user_name" name="user_name" value="${user_name}"  onblur="checkName()" disabled>
...
<form:form modelAttribute="member" method="post" action="insertuser.mb" enctype="multipart/form-data" onsubmit="return validateForm() && validatePassword()">
    ...
    <table border="1">
        <tr>
            <th>* 이메일</th>
            <td>
        <input type="text" id="user_email" name="user_email" value="${member.user_email}" placeholder="이메일을 입력해주세요" onkeyup="checkEmail()">
                <span id="emailMessage" class=""></span>
                <form:errors path="user_email" cssClass="err"/>
            </td>
        </tr>
        ...
        <!-- Nickname Field in Form -->
           ...
           <tr>
            <th>* 이름</th>
            <td>
                <input type="text" id="user_name" name="user_name" value="${member.user_name}" placeholder="이름을 입력해주세요" onblur="checkName()">
                <span id="nameMessage" class=""></span>
                <form:errors path="user_name" cssClass="err"/>
            </td>
        </tr>
...
<script>
    // Spring MVC에서 registerSuccess 속성을 추가하고, 이를 기반으로 처리
    var registerSuccess = ${sessionScope.registerSuccess};

    if (registerSuccess) {
        alert('회원가입이 완료되었습니다. 로그인해주세요.');
        window.location.href = 'loginForm.mb'; // 로그인 페이지 URL로 변경
    }
</script>