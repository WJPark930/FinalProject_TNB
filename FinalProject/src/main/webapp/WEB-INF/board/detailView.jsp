...
$(document).ready(function () {
    ...
    $.ajax({
        ...
        success: function (comments) {
            ...
            if (comments.length === 0) {
                var noCommentsRow = '<tr><td colspan="6">댓글이 없습니다.</td></tr>';
  ...
  //댓글 행 추가하는 함수
    function appendComment(comment) {
        ...
        var commentRow = '<tr>' +
                            ...
                            '<td><button class="comment-recommend-btn" data-comment-id="' + comment.id + '">추천</button></td>' +
                            '<td><button class="comment-delete-btn" data-comment-id="' + comment.id + '">삭제</button></td>' +
                        '</tr>';
     ...
     // 답글 버튼 추가
        var replyButtonRow = '<tr>' +
                                '<td colspan="6"><button class="reply-btn" data-comment-id="' + comment.id + '">답글</button>' +
                                '<button class="toggle-nested-comments-btn" data-comment-id="' + comment.id + '">대댓글 보기/감추기</button></td>' +
                            '</tr>';
        ...
        $.ajax({
            ...
            success: function (nestedComments) {
                ...
                if (nestedComments.length > 0) {
                    var nestedCommentsHtml = '<tr class="nested-comments-row"><td colspan="6"><div class="nested-comments-container">';

                    nestedComments.forEach(function (nestedComment) {
                        var recommendationCount = nestedComment.recommendation_count || 0; // 추천 수가 undefined이면 0으로 설정
                        nestedCommentsHtml += '<div class="nested-comment" data-nested-comment-id="' + nestedComment.id + '">';
                        nestedCommentsHtml += '<p>' + nestedComment.content + ' - ' + nestedComment.user_email + ' - 작성일: ' + formatDate(nestedComment.created_at) + ' - 추천 수: <span class="recommendation-count">' + recommendationCount + '</span>' + '<button class="recommend-button nested-comment-recommend-btn" data-nested-comment-id="' + nestedComment.id + '">추천</button>' + '<button class="delete-nested-comment" data-nested-comment-id="' + nestedComment.id + '">삭제</button>' + '</p>';
                    ...
                    var noCommentsHtml = '<tr class="nested-comments-row"><td colspan="6">답글이 없습니다</td></tr>';
 ...
 // 답글 버튼 클릭 시 답글 입력 폼 토글 처리
    $(document).on('click', '.reply-btn', function () {
        ...
        replyFormRow.toggle(); // 답글 입력 폼 보이기/감추기
    });
    
    // 답글 버튼 클릭 시 답글 입력 폼 표시
    $(document).on('click', '.reply-btn', function () {
        var commentId = $(this).data('comment-id');
        var replyForm = '<tr class="reply-form">' +
                            '<td colspan="6">' +
                                '<form class="nestedCommentForm">' +
                                    '<textarea name="content" rows="2" cols="50" placeholder="답글을 입력하세요"></textarea>' +
...
<body>
		...
		<form>
			...
			<table class="table table-striped" border="1" width="600"
				...
				<tr align="center" height="30">
					<th width="150">글번호</th>
					<td width="150">${content.bid}</td>
					<th width="150" colspan="3">조회수</th>
					<td width="150">${content.readcount}</td>
				</tr>
				<tr align="center" height="30">
					<th width="150">작성자</th>
					<td width="150">${content.user_email}</td>
					<th width="150">작성일</th>
				...
				<tr align="center" height="30">
					<th width="150">글제목</th>
					<td colspan="3" width="450">${content.title}</td>
					<th width="150">추천수</th>
					<td colspan="2" width="450" id="recommendCount">${recommendCount}</td>
				</tr>
				<tr align="center" height="50">
					<th width="150">글내용</th>
					<td colspan="5" width="450" align="left"><br>
						${content.content}</td>
				</tr>
				<tr align="center" height="30">
					<td colspan="6" bgcolor=""><input type="button" value="추천하기"
						id="recommendButton"> <input type="button" value="글수정"
						onClick="location.href='update.bd?bid=${content.bid}&pageNumber=${pageNumber}'">
						<input type="button" value="글삭제"
						onclick="deletePost(${content.bid}, ${pageNumber})"> <input
	...
	<div class="container mt-4">
		<div class="row justify-content-center">
			<div class="col-md-6">
				<div class="card">
					<div class="card-header bg-success text-white">댓글 입력</div>
					<form id="commentForm">
						...
						<div class="card-body">
							...
							<div class="form-group">
								<label>작성자</label> <input type="text"
									...
									value="${sessionScope.loginInfo.user_email}" readonly>
							</div>
						</div>
						<div class="card-footer text-right">
							<button type="submit" class="btn btn-success">댓글 입력</button>
						</div>
					</form>
				</div>
			</div>
	...
	<div class="container">
		<table class="table table-striped table-hover text-center"
			...
			<thead>
				<tr>
					<th colspan="6" class="text-center">댓글 목록</th>
				</tr>
				<tr>
					<th>내용</th>
					<th>작성자</th>
